...
ss_type_find (GstTypeFind * tf, gpointer unused)
{
  ...
  // TODO: Need to add generic handling instead of harding of 288 bytes

  // TODO : add our own logic using xml parsing.. below logic is temporary version
  data = gst_type_find_peek (tf, 0, 289);
  if (!data)
    return;
  ...
  if ((data[0] == 0xff && data[1] == 0xfe) ||
    ...
    GST_INFO ("Manifest file is in UTF 16 format...");
    str = g_utf16_to_utf8 (data, 288, NULL, NULL, NULL);
  } else {
    GST_INFO ("Manifest file is in normal UTF format...");
    str = data;
  }

  if (str) {
    if (!g_strstr_len (str, 288,"SmoothStreamingMedia")) {
      GST_ERROR ("failed to find 'SmoothStreamingMedia' string...");
      return;
    } else {
