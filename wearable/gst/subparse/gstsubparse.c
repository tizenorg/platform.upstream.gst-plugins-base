...
gst_sub_parse_init (GstSubParse * subparse)
{
  ...
  subparse->fps_d = 1001;
  subparse->isSeeking = FALSE;
#ifdef SUBPARSE_MODIFICATION
  subparse->langlist_msg_posted = FALSE;
#endif
}
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_EXTSUB_CURRENT_LANGUAGE:
      ...
      GST_LOG("************setting property to %s*********************", subparse->state.current_language);
...
get_next_sami_sync_line (gchar * line, guint* offset)
{
  ...
  if (line_end == line)
  {
    ...
    if (!line_end ) {
      /* <SYNC not found, or there is only one sync in this line return offset as 0 */
      line_end = tempptr;
      sync_line = (gchar*)g_malloc ((strlen(line_end)) + 1);
      strcpy (sync_line, line_end);
      *(sync_line + strlen(line_end)) = '\0';
      *offset = 0;
      return sync_line;
    }
  }
  ...
  while (!self->flushing && (line = get_next_line (self))) {
    ...
    if (self->parser_type == GST_SUB_PARSE_FORMAT_SAMI) {
      do {
        ...
        if (!self->langlist_msg_posted && self->state.language_list) {
