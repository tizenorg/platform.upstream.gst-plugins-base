...
gboolean gst_camera_control_set_ae_lock(GstCameraControl *control, gint val)
{
	GstCameraControlClass *klass = GST_CAMERA_CONTROL_GET_CLASS(control);

	if (klass->set_ae_lock) {
		return klass->set_ae_lock(control, val);
	}

	return FALSE;
}

gboolean gst_camera_control_set_awb_lock(GstCameraControl *control, gint val)
{
	GstCameraControlClass *klass = GST_CAMERA_CONTROL_GET_CLASS(control);

	if (klass->set_awb_lock) {
		return klass->set_awb_lock(control, val);
	}
