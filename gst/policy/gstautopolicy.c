    ...
    GstPadTemplate * templ, const gchar * name, const GstCaps * caps)
{
  ...
  GstPad *kid_sink_pad = gst_element_get_request_pad (self->kid, "sink_%u");
  it = gst_pad_iterate_internal_links (kid_sink_pad);
  g_assert (it);
  gst_iterator_next (it, &item);
  kid_src_pad = g_value_dup_object (&item);
  g_value_unset (&item);
  g_assert (kid_src_pad);
  ...
  g_object_unref (kid_sink_pad);
  g_object_unref (kid_src_pad);
  return sinkpad;
}
...
gst_auto_policy_class_init (GstAutoPolicyClass * klass)
{
  ...
  gst_element_class_set_static_metadata (gstelement_class,
      "Auto policy", "Policy",
      "Wrapper element for automatically detected policy element",
